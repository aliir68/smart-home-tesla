...
import Toast from 'react-native-toast-message'
 ...
 const EditSensor = (props)=>{
    const{cheng,setCheng,UpdateEditSensor}=useContext(AppContext)
    const [webview,setWebview]=useState(false)
    const [nameSensor,setNameSensor]=useState(props.route.params.values.nameSensor)
    const [iconSensor,setIconSensor]=useState(props.route.params.values.iconSeonsor)
    const [urlSensor,setUrlSensor]=useState(props.route.params.values.urlSensor)
     ...
     return(
        ...
        <ImageBackground  source={require('../file/image/backgroundasli.jpg')} 
            ...
            <View style={{zIndex : 1,position : "relative",top : "18%"}}>
                ...
                <TextInput scrollEnabled={true}  placeholder="tiite / nameOn / nameOff" placeholderTextColor="#fff" color="#fff" style={[styles.TextInput,{paddingLeft: 40}]}  value={iconSensor} onChangeText={(value) =>iconChenged(value)}/>
                {
                ...
                webview == true ?
                    <View style={styles.webview}>
                        <WebView  style={{width : "100%",height :"100%",zIndex : 1,}} 
                            ...
                            <TextInput scrollEnabled placeholder="tiite / nameOn / nameOff" backgroundColor="#888" placeholderTextColor="#fff" color="#fff" style={{paddingLeft: 55}} value={iconSensor} onChangeText={(value) =>iconChenged(value)}/>
